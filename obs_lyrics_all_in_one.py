"""
OBS Lyrics Plugin - All-in-One Version
This combines all functionality into a single script for easier installation
"""
...
def script_description():
    return """OBS Lyrics Plugin (All-in-One)
    
Complete lyrics display system with:
...
Hotkeys:
- Next Line: Move to next lyric
- Previous Line: Move to previous lyric  
- Toggle Visibility: Show/hide lyrics
- Next Song: Switch to next song
- Previous Song: Switch to previous song"""
...
def script_properties():
    ...
    # Song selector
    song_list = obs.obs_properties_add_list(props, "current_song_select", "Current Song",
                                            obs.OBS_COMBO_TYPE_LIST, obs.OBS_COMBO_FORMAT_STRING)
    obs.obs_property_list_add_string(song_list, "-- Select Song --", "")
    
    # Update song list when folder changes
    obs.obs_property_set_modified_callback(
        obs.obs_properties_get(props, "lyrics_folder"),
        lambda props, prop, settings: update_song_list(props, settings)
    )
    ...
    # Scene setup button
    obs.obs_properties_add_button(props, "create_scene", "Create Lyrics Scene", create_lyrics_scene_callback)
    
    # Control buttons
    obs.obs_properties_add_button(props, "prev_line", "◀ Previous Line", lambda props, prop: previous_line(True))
    obs.obs_properties_add_button(props, "next_line", "Next Line ▶", lambda props, prop: next_line(True))
    obs.obs_properties_add_button(props, "toggle_vis", "Toggle Visibility", lambda props, prop: toggle_visibility(True))
    
    return props

def update_song_list(props, settings):
    """Update the song dropdown when folder changes"""
    load_lyrics_from_folder()
    
    song_list = obs.obs_properties_get(props, "current_song_select")
    obs.obs_property_list_clear(song_list)
    obs.obs_property_list_add_string(song_list, "-- Select Song --", "")
    
    for song_name in sorted(lyrics_data.keys()):
        obs.obs_property_list_add_string(song_list, song_name, song_name)
...
def script_update(settings):
    global props_settings, source_name, lyrics_data, current_song
    ...
    selected_song = obs.obs_data_get_string(settings, "current_song_select")
    if selected_song and selected_song != "-- Select Song --":
        select_song(selected_song)
...
def next_song(pressed):
    ...
    if not current_song:
        ...
        try:
            ...
            if idx < len(songs) - 1:
                select_song(songs[idx + 1])
        except ValueError:
            select_song(songs[0])

def previous_song(pressed):
    if not pressed:
        return
...
def script_load(settings):
    ...
    obs.obs_hotkey_register_frontend("lyrics_next_song", "Lyrics: Next Song", next_song)
    obs.obs_hotkey_register_frontend("lyrics_prev_song", "Lyrics: Previous Song", previous_song)
...
def create_lyrics_scene_callback(props, prop):
    """Callback for create scene button"""
    create_lyrics_scene()
    return False
...
def create_lyrics_scene():
    ...
    print("Lyrics scene created successfully!")
